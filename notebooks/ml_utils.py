...
def rf_fire_grid_run(clim_grid_df, rb_frac, n_features= 36, dropcols= ['RH', 'Ant_RH'], n_trees= 100, threshold= 0.4, criterion= 'gini', test_data= True):
    
    # Creates a RF classifier instance for predicting fire probabilities at the grid scale
    
    rseed= np.random.randint(100)
    
    df1= clim_grid_df[clim_grid_df['fire_freq']==1].iloc[0:10000]
    n1= len(df1)
    n2= rb_frac*n1
    df2= clim_grid_df[clim_grid_df['fire_freq']==0]
    ...
    if test_data:
        df3= clim_grid_df[clim_grid_df['fire_freq']==1].iloc[10000:]
        df4= clim_grid_df[clim_grid_df['fire_freq']==0]
...
def rf_hyperparam_tuning(clim_df, dropcols= ['Solar', 'Ant_Tmax', 'RH', 'Ant_RH'], rb_frac_arr= [10, 4, 3, 7/3, 3/2, 1], n_trees_arr= [50, 100, 250, 500, 1000], \
                         thresh_arr= [0.4, 0.5, None], n_iters= 5, run_id= None, modsave= False):
        ...
        for n_trees in n_trees_arr:
            for rb_frac in rb_frac_arr:
                for thresh in thresh_arr:
                    rf, forest, train_metrics, test_metrics= rf_fire_grid_run(clim_grid_df= clim_df, rb_frac= rb_frac, dropcols= dropcols, n_trees= n_trees, threshold= thresh)
    ...
    param_df= pd.DataFrame(list_of_lists, columns=["Iteration", "Trees", "Rebalance frac", "Threshold", "Train Accuracy", "Train Recall", "Test Accuracy", "Test Recall"])
    return param_df
=======
    ...
    if loco:
        ...
        if debug:
            ...
            burnarea_df= size_pred_func(mdn, stat_model, size_test_df, X_test_dat, max_size_arr, sum_size_arr, ncomps= n_comps, freq_flag= freq_flag, \
                                            regmlfreq= regmlfreq, freqs_data= freqs_data, debug= False, regindx= regindx, seed= rseed)
            return burnarea_df
>>>>>>> ce3e78c6ccdf9610a36aeea6d77578f7e6ba9b19
